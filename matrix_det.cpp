...
long double triangularMatrix(vector <vector <long double>>& mas, int size);
...
int main() {
    ...
    start = clock();
    long double det_1;
    det_1 = det_rec(matrix, N, column);
    cout << "\nОпределитель матрицы: " << det_1 << endl;
    finish = clock();
    ...
    start = clock();
    long double det_t;
    det_t = triangularMatrix(matrix, N);
    cout << "\nОпределитель матрицы: " << det_t << endl;
    finish = clock();
    cout << "\nВремя выполнения: " << (double)(finish - start) / 1000 << endl;

    printMatrix(matrix, N);
    cout << "\n Разница: " << det_t - det_1 << endl;
